...
graph TD
    subgraph "你會建造的系統"
        direction TB
        Apps["📱 Apps<br/>NotesApp, MusicApp, CrashyApp"]
        Framework["⚙️ Framework Services<br/>ActivityManager, PackageManager, Intents, Broadcasts"]
        Runtime["🔄 Runtime Contract<br/>Lifecycle callbacks, Looper event loop"]
        Native["🔧 Native Layer<br/>Binder IPC, init, Zygote, servicemanager"]
        Kernel["🐧 Linux Kernel<br/>fork, exec, socket, epoll, signals"]
...
```mermaid
graph LR
    S0["<b>S0</b><br/>Build + Looper<br/>+ Parcel"]
    S1["<b>S1</b><br/>init 強化<br/>crash-restart"]
    S2["<b>S2</b><br/>Binder IPC<br/>+ death notify"]
    S3["<b>S3</b><br/>servicemanager<br/>Binder 版"]
    S4["<b>S4</b><br/>system_server<br/>+ managers"]
    S5["<b>S5</b><br/>Zygote<br/>fork+specialize"]
    S6["<b>S6</b><br/>App lifecycle<br/>7 callbacks"]
    S7["<b>S7</b><br/>PMS + Intents"]
    S8["<b>S8</b><br/>AMS + lmkd<br/>+ Broadcasts"]
...
graph LR
    ...
    subgraph todo ["📋 待完成"]
        E["<b>Step 0B</b><br/>C++ Looper<br/>(epoll)"]
        F["<b>Step 0C</b><br/>Kotlin Looper<br/>(Handler+MessageQueue)"]
        G["<b>Step 0D</b><br/>Binary Parcel<br/>(C++ ↔ Kotlin)"]
    end
...
graph TB
    subgraph "Stage 0 完成狀態"
        L_CPP["C++ Looper<br/>epoll + timer + fd"]
        L_KT["Kotlin Looper<br/>Handler + MessageQueue"]
        P_CPP["C++ Parcel<br/>binary read/write"]
        P_KT["Kotlin Parcel<br/>binary read/write"]
        BUILD["Build system<br/>Makefile + scripts"]
        HELLO["Hello World<br/>PING/PONG prototype"]
    end

    P_CPP <-->|"相同 wire format"| P_KT
    L_CPP -.->|"概念相同<br/>實作獨立"| L_KT
```
...
sequenceDiagram
    ...
    I->>I: 記錄 crash 次數<br/>計算 backoff: 1s, 2s, 4s...
    I->>SM: fork+exec (重啟!)
    SM->>SM: 恢復運行 ✓
```
...
graph TB
    subgraph "Stage 1 完成狀態"
        INIT["init<br/>✅ 解析 init.rc<br/>✅ fork+exec<br/>✅ crash-restart + backoff<br/>✅ property store"]
        SM["servicemanager"]
        SS["system_server"]
        PROP["property.sock<br/>getprop/setprop"]
    end
...
graph LR
    subgraph "沒有 Binder 的世界"
        A1[App] -->|"自己開 socket<br/>自己定 protocol<br/>自己 parse"| S1[Service]
    end

    subgraph "有 Binder 的世界"
        A2[App] -->|"service.getValue()"| P[Proxy]
        P -->|"自動序列化<br/>自動傳輸<br/>自動反序列化"| ST[Stub]
...
sequenceDiagram
    ...
    participant T as Transport Layer<br/>(Unix socket)
    participant SS as Server Stub
    participant S as Server Implementation

    C->>CP: proxy.ping("hello")
    Note over CP: 1. 寫 method code 到 Parcel<br/>2. 寫參數 "hello" 到 Parcel
    CP->>T: send(handle, parcel)
    Note over T: 3. 用 handle 查出目標 socket<br/>4. 附上 caller UID/PID<br/>5. write() 到 socket
    T->>SS: dispatch to binder thread
    Note over SS: 6. 從 Parcel 讀出 method code<br/>7. 從 Parcel 讀出參數
    SS->>S: ping("hello")
    S-->>SS: return "pong"
...
graph LR
    subgraph "Stage 2 步驟"
        A["<b>Step 2A</b><br/>Transport 層<br/>socket 連線 + handle"]
        B["<b>Step 2B</b><br/>Proxy/Stub 模式<br/>手寫版"]
        C["<b>Step 2C</b><br/>AIDL Codegen<br/>自動產生 Proxy/Stub"]
        D["<b>Step 2D</b><br/>Thread Pool<br/>+ linkToDeath"]
    end
...
graph LR
    subgraph "Client Process"
        C[Client code] -->|"proxy.add(3, 5)"| P["Proxy<br/>(自動序列化)"]
    end

    P -->|"Parcel: [code=1, 3, 5]"| Network["Transport<br/>(socket)"]

    subgraph "Server Process"
        Network -->|"Parcel"| S["Stub<br/>(自動反序列化)"]
        S -->|"add(3, 5)"| I["Implementation<br/>return 8"]
    end
```
...
2. **手寫 Proxy**（client 側）：
   ```kotlin
   class CalculatorProxy(private val handle: Int) : ICalculator {
...
3. **手寫 Stub**（server 側）：
   ```kotlin
   abstract class CalculatorStub : ICalculator {
...
4. **寫 Implementation**：
   ```kotlin
   class CalculatorImpl : CalculatorStub() {
...
5. **兩個 process 測試**：
   - Server process 建立 CalculatorImpl，監聽 socket
   - Client process 建立 CalculatorProxy，呼叫 `add(3, 5)` → 得到 8
...
2. **codegen.py 輸入/輸出**：
   ...
   - `ICalculatorProxy.kt` — client 側 Proxy
   - `ICalculatorStub.kt` — server 側 Stub
...
# 2. 檢查產生的檔案
cat frameworks/base/core/kotlin/generated/ICalculatorProxy.kt
...
sequenceDiagram
    ...
    Note over C: callback 被呼叫：<br/>「calculator 掛了，我要 reconnect」
```
...
graph TB
    subgraph "Stage 2 完成狀態"
        T["Transport 層<br/>handle-based addressing<br/>caller UID/PID"]
        PS["Proxy/Stub<br/>自動序列化/反序列化"]
        AIDL["AIDL codegen<br/>.aidl → Kotlin code"]
        TP["Thread Pool<br/>4 binder threads"]
        LD["linkToDeath<br/>crash notification"]
        P["Binary Parcel<br/>(from Stage 0)"]
    end
...
graph LR
    subgraph "Stage 0（目前）"
        A1["system_server"] -->|"ADD_SERVICE ping /path\n"| SM1["servicemanager<br/>(text protocol)"]
        A2["HelloApp"] -->|"GET_SERVICE ping\n"| SM1
    end

    subgraph "Stage 3（目標）"
        B1["system_server"] -->|"Binder transaction<br/>code=ADD_SERVICE<br/>Parcel{name, handle}"| SM2["servicemanager<br/>(Binder, handle 0)"]
        B2["HelloApp"] -->|"Binder transaction<br/>code=GET_SERVICE<br/>Parcel{name}"| SM2
    end
```
...
2. **跑 codegen** 產生 `IServiceManagerProxy.kt` + `IServiceManagerStub.kt`
...
4. **Kotlin client helper `ServiceManager.kt`：**
   ...
   ```
   內部用 `IServiceManagerProxy(handle=0)` 跟 servicemanager 通訊。
...
sequenceDiagram
    ...
    Note over App: 下次呼叫 ping 時<br/>發現 handle 無效<br/>→ 重新 getService()
```
...
graph TB
    subgraph "Stage 3 完成狀態"
        SM["servicemanager<br/>✅ Binder-based (handle 0)<br/>✅ addService / getService / listServices<br/>✅ Crash → restart → re-registration"]
        SS["system_server<br/>✅ linkToDeath(SM)<br/>✅ auto re-register"]
        INIT["init<br/>✅ crash-restart SM"]
    end
...
graph TB
    subgraph SS["system_server process"]
        AMS["ActivityManagerService<br/>App 生命週期管理"]
        PMS["PackageManagerService<br/>App 安裝 + Intent 解析"]
        PROP["PropertyManagerService<br/>系統設定 key-value"]
    end
...
graph TB
    subgraph SS["system_server"]
        AMS["AMS<br/>getCurrentState()<br/>forceStopPackage()<br/>getRunningProcesses()"]
        PMS["PMS<br/>getInstalledPackages()<br/>getUidForPackage()"]
        PROP["PropertyManager<br/>get/set/list"]
    end
...
```mermaid
graph TD
    Z["Zygote<br/>(已載入 framework classes)"]
    Z -->|"fork()"| A1["App 1<br/>共享 framework code<br/>(copy-on-write)"]
    Z -->|"fork()"| A2["App 2<br/>共享 framework code<br/>(copy-on-write)"]
    Z -->|"fork()"| A3["App 3<br/>共享 framework code<br/>(copy-on-write)"]
...
1. **App process 啟動順序：**
   ...
   Zygote fork → exec java -jar app.jar
     → ActivityThread.main()
       ...
       → 連到 servicemanager
       → 連到 AMS，呼叫 attachApplication(pid)
       → AMS 回覆「啟動這個 Activity」
...
2. **ActivityThread.kt：**
   ...
   object ActivityThread {
       ...
       fun main(args: Array<String>) {
           ...
           ams.attachApplication(ProcessHandle.current().pid())

           // 3. 進入 event loop
           Looper.loop()
       }

       // AMS 透過 Binder 呼叫這些方法（在 binder thread 上）
       // 用 Handler.post 轉到 main thread
       fun scheduleLaunchActivity(activityName: String, savedState: Bundle?) {
           mainHandler.post {
...
3. **AMS 端的 `attachApplication()`：**
   - 記錄 pid → package 的映射
   - 查看是否有待啟動的 Activity → 呼叫 `scheduleLaunchActivity()`
...
graph TD
    ...
    subgraph "Hybrid（最複雜）"
        ...
        H3["stopSelf() 呼叫了<br/>AND<br/>所有 client unbind"]
        H4["才會 onDestroy()"]
        H1 --> H2 --> H3 --> H4
    end
```
...
```mermaid
graph LR
    FG["FOREGROUND<br/>oom_adj=0<br/>最後被 kill"] --- VIS["VISIBLE<br/>100"]
    VIS --- PERC["PERCEPTIBLE<br/>200"]
    PERC --- SVC["SERVICE<br/>500"]
    SVC --- CACHED["CACHED<br/>900-999<br/>最先被 kill"]
...
graph TB
    subgraph "Stage 6 完成狀態"
        AT["ActivityThread<br/>main looper + AMS attach"]
        ACT["Activity<br/>7 callbacks + save/restore"]
        SVC["Service<br/>started + bound + hybrid<br/>restart policy"]
        OOM["OomAdjuster<br/>5 級 priority<br/>binding inheritance"]
    end
...
graph LR
    subgraph "Explicit Intent（明確指定）"
        E1["startActivity(<br/>package=com.app.notes<br/>class=NotesActivity)"]
        E1 -->|"直接找到"| E2["NotesApp.NotesActivity"]
    end

    subgraph "Implicit Intent（按 action 配對）"
        I1["startActivity(<br/>action=ACTION_SEND<br/>data='Hello')"]
        I1 -->|"PMS 查 manifest<br/>誰註冊了 ACTION_SEND？"| I2["ShareApp? ReceiverApp?"]
...
graph TB
    subgraph "Stage 8 完成後的系統"
        ...
        subgraph SS["system_server"]
            AMS["AMS<br/>lifecycle + fg/bg<br/>+ force-stop"]
            PMS["PMS<br/>manifest + intent"]
            PROP["PropertyManager"]
            LMKD["lmkd<br/>memory pressure<br/>→ kill by oom_adj"]
            BC["BroadcastDispatcher<br/>BOOT_COMPLETED<br/>LOW_MEMORY"]
        end
...
3. **lmkd 跟 AMS 的互動：**
   ...
   sequenceDiagram
       ...
       L->>AMS: getProcessesToKill(level=LOW)
       AMS->>AMS: 排序 LRU list by oom_adj
       AMS-->>L: [NotesApp(adj=999), MusicApp(adj=900)]

       L->>L: kill(NotesApp.pid, SIGKILL)
       L->>AMS: processKilled(NotesApp.pid)
       AMS->>AMS: cleanup ProcessRecord
       AMS->>AMS: 保管 NotesApp 的 savedState
       Note over AMS: 下次啟動 NotesApp 時<br/>用 savedState 恢復
   ```
...
graph TB
    subgraph "Phase 1 完成的完整系統"
        direction TB
        INIT["init<br/>rc parser, crash-restart<br/>property store"]
        SM["servicemanager<br/>Binder handle 0"]
        Z["Zygote<br/>preload, fork, specialize"]

        subgraph SS["system_server"]
            AMS["AMS<br/>lifecycle, fg/bg, oom_adj<br/>force-stop, LRU"]
            PMS["PMS<br/>manifest scan, UID<br/>intent resolve"]
            PROP["PropertyManager"]
            LMKD["lmkd<br/>memory pressure → kill"]
            BCD["BroadcastDispatcher<br/>BOOT_COMPLETED<br/>LOW_MEMORY"]
        end
